...
class RepBottleneck(Bottleneck):
    ...
    def build(self, input_shape):
        ...
        self.conv1 = RepSimpleBlock(hidden_dim, self.kernels[0], activation=self.activation, normalizer=self.normalizer)
...
class ResNBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.c     = input_shape[-1]
        hidden_dim = int(self.filters * self.expansion)
        self.conv1 = ConvolutionBlock(hidden_dim, (1, 1), activation=self.activation, normalizer=self.normalizer)
        self.conv2 = ConvolutionBlock(hidden_dim, (3, 3), activation=self.activation, normalizer=self.normalizer)
        self.conv3 = ConvolutionBlock(self.filters, (1, 1), downsample=self.downsample, groups=self.groups, activation=self.activation, normalizer=self.normalizer)
...
class RepResNBlock(ResNBlock):
    ...
    def build(self, input_shape):
        ...
        hidden_dim = int(self.filters * self.expansion)
        self.conv1 = ConvolutionBlock(hidden_dim, (1, 1), activation=self.activation, normalizer=self.normalizer)
        self.conv2 = RepSimpleBlock(hidden_dim, (3, 3), activation=self.activation, normalizer=self.normalizer)
        self.conv3 = ConvolutionBlock(self.filters, (1, 1), downsample=self.downsample, groups=self.groups, activation=self.activation, normalizer=self.normalizer)
...
class BottleneckCSPA2(BottleneckCSPA):
    ...
    def build(self, input_shape):
        ...
        self.block = Sequential([
            Bottleneck2(hidden_dim, groups=self.groups, shortcut=self.shortcut, activation=self.activation, normalizer=self.normalizer) for i in range(self.iters)
        ])
...
class BottleneckCSP2(BottleneckCSP):
    ...
    def build(self, input_shape):
        ...
        self.middle = Sequential([
            Bottleneck2(hidden_dim, shortcut=self.shortcut, activation=self.activation, normalizer=self.normalizer) for i in range(self.iters)
        ])
...
class RepNCSP(BottleneckCSPA):
    ...
    def build(self, input_shape):
        ...
        hidden_dim = int(self.filters * self.expansion)
        self.block = Sequential([
            RepBottleneck(hidden_dim, kernels=(3, 3), groups=self.groups, shortcut=self.shortcut, activation=self.activation, normalizer=self.normalizer) for i in range(self.iters)
        ])
...
class SPPELAN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if isinstance(self.filters, (tuple, list)):
            ...
            f0 = f1 = self.filters
        self.conv1 = ConvolutionBlock(f0, (1, 1), activation=self.activation, normalizer=self.normalizer)
        self.block = [MaxPooling2D(pool_size=(5, 5), strides=(1, 1), padding="SAME"),
                      MaxPooling2D(pool_size=(5, 5), strides=(1, 1), padding="SAME"),
                      MaxPooling2D(pool_size=(5, 5), strides=(1, 1), padding="SAME")]
        self.conv2 = ConvolutionBlock(f1, (1, 1), activation=self.activation, normalizer=self.normalizer)
...
class RepNCSPELAN4(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.block = [
            Sequential([
                RepNCSP(f1, 
                        ...
                        activation = self.activation, 
                        normalizer = self.normalizer),
                ConvolutionBlock(f1, (3, 3), activation=self.activation, normalizer=self.normalizer)
            ]),
            Sequential([
                RepNCSP(f1, 
                        groups     = self.groups, 
        ...
        self.conv2 = ConvolutionBlock(f2, (1, 1), activation=self.activation, normalizer=self.normalizer)
...
def DarkNetELAN4_A(down_block,
    ...
    # conv down
    x = ConvolutionBlock(f0, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stem.block1')(img_input)
                       
    # conv down
    x = ConvolutionBlock(f1, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage1.block1')(x)
    ...
    if down_block == AverageConvolutionDown:
        x = down_block(f2, (3, 3), activation=activation, normalizer=normalizer, name='stage2.block1')(x)
    elif down_block == ConvolutionBlock:
        x = down_block(f2, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage2.block1')(x)
    ...
    # conv down
    if down_block == AverageConvolutionDown:
        x = down_block(f3, (3, 3), activation=activation, normalizer=normalizer, name='stage3.block1')(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f3, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage3.block1')(x)
    ...
    # conv down
    if down_block == AverageConvolutionDown:
        x = down_block(f3, (3, 3), activation=activation, normalizer=normalizer, name='stage4.block1')(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f3, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage4.block1')(x)
...
def DarkNetELAN4_B(down_block,
    ...
    x = ConvolutionBlock(f0, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stem.branch1.block1')(img_input)
    y0 = ConvolutionBlock(f0, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stem.branch2.block1')(img_input)

    x = ConvolutionBlock(f1, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage1.branch1.block1')(x)
    y1 = CBLinear([f0], name='stage1.branch2.block1')(x)
    ...
    x = RepNCSPELAN4(filters    = [f1, f0, f2], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name="stage1.branch1.block2")(x)
    y2 = CBLinear([f0, f1], name='stage1.branch2.block2')(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f2, (3, 3), activation=activation, normalizer=normalizer, name='stage2.branch1.block1')(x)
    elif down_block == ConvolutionBlock:
        x = down_block(f2, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage2.branch1.block1')(x)
    ...
    x = RepNCSPELAN4(filters    = [f2, f1, f3], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name='stage2.branch1.block2')(x)
    y3 = CBLinear([f0, f1, f2], name='stage2.branch2.block1')(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f3, (3, 3), activation=activation, normalizer=normalizer, name='stage3.branch1.block1')(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f3, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage3.branch1.block1')(x)
    ...
    x = RepNCSPELAN4(filters    = [f3, f2, f4], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name='stage3.branch1.block2')(x)
    y4 = CBLinear([f0, f1, f2, f3], name='stage3.branch2.block1')(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f4, (3, 3), activation=activation, normalizer=normalizer, name='stage4.branch1.block1')(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f4, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage4.branch1.block1')(x)
    ...
    x = RepNCSPELAN4(filters    = [f3, f2, f4], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name='stage4.branch1.block2')(x)
    y5 = CBLinear([f0, f1, f2, f3, f4], name='stage4.branch2.block1')(x)

    x = CBFuse(fuse_index=[0, 0, 0, 0, 0], name='stage5.block1')([y0, y1, y2, y3, y4, y5])

    x = ConvolutionBlock(f1, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name='stage6.block1')(x)
    ...
    x = RepNCSPELAN4(filters    = [f1, f0, f2], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name='stage6.block3')(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f2, (3, 3), activation=activation, normalizer=normalizer, name="stage7.block1")(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f2, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name="stage7.block1")(x)
    ...
    x = RepNCSPELAN4(filters    = [f2, f1, f3], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name="stage7.block3")(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f3, (3, 3), activation=activation, normalizer=normalizer, name="stage8.block1")(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f3, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name="stage8.block1")(x)
    ...
    x = RepNCSPELAN4(filters    = [f3, f2, f4], 
                     ...
                     activation = activation,
                     normalizer = normalizer,
                     name="stage8.block3")(x)

    if down_block == AverageConvolutionDown:
        x = down_block(f4, (3, 3), activation=activation, normalizer=normalizer, name="stage9.block1")(x)
    elif down_block == ConvolutionBlock:
        x = ConvolutionBlock(f4, (3, 3), downsample=True, activation=activation, normalizer=normalizer, name="stage9.block1")(x)
...
def DarkNetELAN4_large_backbone(input_shape=(640, 640, 3),
                               include_top=False, 
                               weights='imagenet', 
                               activation='leaky-relu',
                               normalizer='batch-norm',
    ...
    model = DarkNetELAN4_large(include_top=include_top, 
                              weights=weights,
                              activation=activation,
                              normalizer=normalizer,
                              input_shape=input_shape)
    ...
    if custom_layers is not None:
        ...
        y_2 = model.get_layer("stage5.block1").output
        y_4 = model.get_layer("stage6.block3").output
        y_8 = model.get_layer("stage7.block3").output
        y_16 = model.get_layer("stage8.block3").output
        y_32 = model.get_layer("stage9.block3").output
        return Model(inputs=model.inputs, outputs=[y_2, y_4, y_8, y_16, y_32], name=model.name + '_backbone')