...
def camera_flare(metadata, radius=0.5, alpha=0.6):
    assert 0 < radius <= 1
    ...
    blur_size = int(random.uniform(0.15, 0.25) * avg_dim)
    circle = cv2.blur(circle, (blur_size, blur_size))

    image = cv2.addWeighted(image, 1.0, circle, alpha, 0.0)
...
class CameraFlare(BaseTransform):
    def __init__(self, radius=0.5, alpha=0.6):
        self.radius = radius
        self.alpha = alpha
...
class RandomCameraFlare(BaseRandomTransform):
    def __init__(self, radius=0.5, alpha=0.6, prob=0.5):
        self.radius = radius
        self.alpha = alpha
        self.prob = prob
