...
def erasing(image, 
            min_area=0.02, max_area=1/3, 
            min_aspect=0.3, max_aspect=None, 
            min_count=1, max_count=1, 
            fill=0, mode='constant'):

    def _get_pixels(img, size, mode, fill):
        if mode == "random":
            fill = [random.randint(0, 255) for _ in range(img.shape[-1])]
        return np.full(size, fill_value=fill)
    ...
    for _ in range(count):
        for attempt in range(10):
            ...
            if h < img.shape[0] and w < img.shape[1]:
                ...
                img[top:top + h, left:left + w, :] = _get_pixels(img, (h, w, img.shape[-1]), mode, fill)
                break
    return img
